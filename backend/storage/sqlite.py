...
class SqliteRepository(Repository):
            ...
            self._conn.execute(
                ...
                INSERT INTO opportunities
                    (id, customer_name, state, product, priority, qualification,
                     human_takeover, updated_at, payload)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def get_opportunity(self, opportunity_id: str) -> Optional[Opportunity]:
        ...
        return codec.opportunity_from_dict(json.loads(row["payload"])) if row else None
    ...
    def _write_case(self, case: HumanCase) -> None:
        with self._lock:
            self._conn.execute(
                """
                INSERT INTO cases (id, opportunity_id, status, created_at, payload)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                    opportunity_id=excluded.opportunity_id, status=excluded.status,
                ...
                (
                    ...
                    _dumps(codec.case_to_dict(case)),
                ),
            )
            self._conn.commit()
        ...
        with self._lock:
            self._conn.execute(
                ...
                INSERT OR REPLACE INTO message_receipts
                    (opportunity_id, client_message_id, schema_version, document, created_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    opportunity_id,
