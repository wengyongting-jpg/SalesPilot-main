...
def _service(model):
    ...
    runtime = AgentRuntime(kb=loader.load(), model=model, max_tool_calls=6)
...
class TestInteractiveSetup(unittest.TestCase):
    """`--configure` writes the user's file. It must not damage it."""

    def _write(self, existing: str | None, settings: dict) -> str:
        ...
        with TemporaryDirectory() as directory:
            ...
            try:
                configure._write_env(settings)
                return path.read_text(encoding="utf-8")
            finally:
                config.ENV_FILE = original

    def test_comments_and_unmanaged_settings_survive(self):
        """The file belongs to the user. Regenerating it from the template would be
        easier and would silently delete whatever else they had put there."""
        written = self._write(
            "# my own note\n"
            "SALESPILOT_LLM=offline\n"
            "SALESPILOT_PORT=8123\n"
            "SALESPILOT_LOG_LEVEL=DEBUG\n",
            {"SALESPILOT_LLM": "gateway"},
        )
        self.assertIn("# my own note", written)
        self.assertIn("SALESPILOT_PORT=8123", written)
        self.assertIn("SALESPILOT_LOG_LEVEL=DEBUG", written)
        self.assertIn("SALESPILOT_LLM=gateway", written)

    def test_a_managed_key_is_replaced_rather_than_duplicated(self):
        written = self._write(
            "SALESPILOT_LLM=offline\n", {"SALESPILOT_LLM": "gateway"}
        )
        lines = [
            line for line in written.splitlines()
            if line.startswith("SALESPILOT_LLM=")
        ]
        self.assertEqual(["SALESPILOT_LLM=gateway"], lines)

    def test_a_setting_not_yet_present_is_appended(self):
        written = self._write(
            "SALESPILOT_PORT=8000\n", {"SALESPILOT_LLM_MODEL": "gpt-4o-mini"}
        )
        self.assertIn("SALESPILOT_LLM_MODEL=gpt-4o-mini", written)
        self.assertIn("SALESPILOT_PORT=8000", written)

    def test_a_commented_out_setting_is_not_treated_as_the_live_one(self):
        """Writing over a comment would leave the real setting untouched below it and
        the file self-contradictory."""
        written = self._write(
            "# SALESPILOT_LLM=openai\nSALESPILOT_LLM=offline\n",
            {"SALESPILOT_LLM": "gateway"},
        )
        self.assertIn("# SALESPILOT_LLM=openai", written)
        self.assertIn("SALESPILOT_LLM=gateway", written)

    def test_the_file_always_ends_with_exactly_one_newline(self):
        written = self._write("SALESPILOT_LLM=offline", {"SALESPILOT_LLM": "gateway"})
        self.assertTrue(written.endswith("\n"))
        self.assertFalse(written.endswith("\n\n"))

    def test_what_it_writes_is_what_the_loader_reads_back(self):
        """The two halves have to agree, or setup reports success and startup
        disagrees."""
        from backend import configure
        ...
        with TemporaryDirectory() as directory:
            ...
            before = dict(os.environ)
            try:
                configure._write_env({
                    "SALESPILOT_LLM": "gateway",
                    "SALESPILOT_LLM_API_KEY": "sk-round-trip-value",
                    "SALESPILOT_LLM_MODEL": "gpt-4o-mini",
                    "SALESPILOT_LLM_BASE_URL": "https://example.test/v1",
                })
                for name in configure.MANAGED_KEYS:
                    os.environ.pop(name, None)
                config.load_env_file(path)
                self.assertEqual("gateway", os.environ["SALESPILOT_LLM"])
                self.assertEqual(
                    "sk-round-trip-value", os.environ["SALESPILOT_LLM_API_KEY"]
                )
                self.assertEqual(
                    "https://example.test/v1", os.environ["SALESPILOT_LLM_BASE_URL"]
                )
            finally:
                config.ENV_FILE = original
                os.environ.clear()
...
class TestModelOrdering(unittest.TestCase):
    """A provider can return several hundred names. The first one offered matters."""

    CATALOGUE = [
        "text-embedding-3-small", "whisper-1", "dall-e-3", "gpt-4o",
        "gpt-4o-mini", "gpt-4o-realtime-preview", "o3-mini", "claude-3-5-sonnet",
    ]

    def _ordered(self) -> list[str]:
        from backend.configure import _order_models

        return _order_models(self.CATALOGUE)

    def test_a_usable_chat_model_is_suggested_first(self):
        self.assertEqual("gpt-4o-mini", self._ordered()[0])

    def test_nothing_is_dropped(self):
        self.assertCountEqual(self.CATALOGUE, self._ordered())

    def test_models_that_cannot_chat_are_pushed_to_the_bottom(self):
        ordered = self._ordered()
        for name in ("text-embedding-3-small", "whisper-1", "dall-e-3"):
            with self.subTest(name=name):
                self.assertGreater(
                    ordered.index(name), ordered.index("claude-3-5-sonnet")
                )

    def test_a_realtime_variant_is_not_promoted_by_a_substring_match(self):
        """`gpt-4o` is a preferred name and a substring of `gpt-4o-realtime-preview`,
        which cannot serve an ordinary chat completion. Promoting it would put a model
        that fails on first use at the top of the list."""
        ordered = self._ordered()
        self.assertGreater(
            ordered.index("gpt-4o-realtime-preview"), ordered.index("o3-mini")
        )

    def test_an_unrecognised_catalogue_is_returned_intact(self):
        from backend.configure import _order_models

        names = ["zeta-1", "alpha-2"]
        self.assertCountEqual(names, _order_models(names))
