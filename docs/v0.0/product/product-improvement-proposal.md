...
Status: **implemented for local demo; verification in progress**. This is a cross-track product plan, not a replacement for the backend plan or frontend changelog. The current v1 API contract remains frozen; accepted wire additions are recorded in `docs/v0.0/api/interface-v2.md`.
