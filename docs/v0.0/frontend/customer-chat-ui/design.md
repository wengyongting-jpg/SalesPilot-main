...
(`docs/v0.0/backend/backend-contract.md` Part B). The rule is: **detect, do not assume.**
...
- There is no authentication; the `customer_id` is client-supplied. Any client can
  ...
  `product.md` and `docs/v0.0/backend/backend-contract.md`; not to be silently "fixed" here.
