...
- <items from docs/v0.0/backend/backend-contract.md this relies on, and the degradation used
...
- **Customer slice 4 deferred.** The `salespilot` adapter is still a stub, held
  until `docs/v0.0/api/interface-v1.md` is frozen, because interface §5.1 moves the
...
Current state of `docs/v0.0/backend/backend-contract.md` and the degradation in force:
