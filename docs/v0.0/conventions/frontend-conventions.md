...
- `salespilot.js` must work against **today's** API and tolerate the **target**
  API from `docs/v0.0/backend/backend-contract.md`. Treat every field added by the contract as
...
- Never let a missing backend capability block a frontend task. Degrade, and note
  the gap in `docs/v0.0/backend/backend-contract.md`.
...
`docs/v0.0/frontend/frontend-changelog.md` is updated **only when the user explicitly asks**.
